...
OperationMetaDataEnabled {
    ...
    public AddMessageProcessor(String operationName) {
        super(operationName);
    }
    ...
    public MuleEvent doProcess(MuleEvent event) throws Exception {
        ...
        String _transformedOaObject = (String)this.evaluateAndTransform(this.getMuleContext(), event, AddMessageProcessor.class.getDeclaredField("_oaObjectType").getGenericType(), null, this.oaObject);
        XMLStreamReader _transformedRequest = (XMLStreamReader)this.evaluateAndTransform(this.getMuleContext(), event, AddMessageProcessor.class.getDeclaredField("_requestType").getGenericType(), null, this.request);
    ...
    public Result<MetaData> getInputMetaData() {
        if (this.oaObject == null || this.oaObject.toString() == null) {
            return new DefaultResult(null, Result.Status.FAILURE, "There was an error retrieving metadata from parameter: oaObject at processor add at module OpenAirConnector");
        }
        DefaultMetaDataKey metaDataKey = new DefaultMetaDataKey(this.oaObject.toString(), null);
        metaDataKey.setCategory("AddMetaData");
        metaDataKey.addProperty((MetaDataKeyProperty)new TypeDescribingProperty(TypeDescribingProperty.TypeScope.INPUT, "add"));
        ...
        keyForStudio.setCategory("AddMetaData");
    ...
    public Result<MetaData> getOutputMetaData(MetaData inputMetadata) {
        ...
        DefaultMetaDataKey metaDataKey = new DefaultMetaDataKey(this.oaObject.toString(), null);
        metaDataKey.setCategory("AddMetaData");
        metaDataKey.addProperty((MetaDataKeyProperty)new TypeDescribingProperty(TypeDescribingProperty.TypeScope.OUTPUT, "add"));
        Result<MetaData> genericMetaData = this.getGenericMetaData((MetaDataKey)metaDataKey);
        if (Result.Status.FAILURE.equals((Object)genericMetaData.getStatus())) {
            return genericMetaData;
        }
        ...
        try {
            ...
            try {
                ...
                if (metadata.get() == null) {
                    return new DefaultResult(null, Result.Status.FAILURE, "There was an error processing metadata at OpenAirConnector at add retrieving was successful but result is null");
                }
                return metadata;
            }
            catch (Exception e) {
