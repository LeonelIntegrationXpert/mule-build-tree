...
OperationMetaDataEnabled {
    ...
    public MuleEvent doProcess(MuleEvent event) throws Exception {
        ...
        String _transformedOaObject = (String)this.evaluateAndTransform(this.getMuleContext(), event, DeleteMessageProcessor.class.getDeclaredField("_oaObjectType").getGenericType(), null, this.oaObject);
        XMLStreamReader _transformedRequest = (XMLStreamReader)this.evaluateAndTransform(this.getMuleContext(), event, DeleteMessageProcessor.class.getDeclaredField("_requestType").getGenericType(), null, this.request);
    ...
    public Result<MetaData> getInputMetaData() {
        if (this.oaObject == null || this.oaObject.toString() == null) {
            return new DefaultResult(null, Result.Status.FAILURE, "There was an error retrieving metadata from parameter: oaObject at processor delete at module OpenAirConnector");
        }
        DefaultMetaDataKey metaDataKey = new DefaultMetaDataKey(this.oaObject.toString(), null);
        metaDataKey.setCategory("DeleteMetaData");
        metaDataKey.addProperty((MetaDataKeyProperty)new TypeDescribingProperty(TypeDescribingProperty.TypeScope.INPUT, "delete"));
        ...
        keyForStudio.setCategory("DeleteMetaData");
    ...
    public Result<MetaData> getOutputMetaData(MetaData inputMetadata) {
        ...
        DefaultMetaDataKey metaDataKey = new DefaultMetaDataKey(this.oaObject.toString(), null);
        metaDataKey.setCategory("DeleteMetaData");
        metaDataKey.addProperty((MetaDataKeyProperty)new TypeDescribingProperty(TypeDescribingProperty.TypeScope.OUTPUT, "delete"));
        Result<MetaData> genericMetaData = this.getGenericMetaData((MetaDataKey)metaDataKey);
        if (Result.Status.FAILURE.equals((Object)genericMetaData.getStatus())) {
            return genericMetaData;
        }
        ...
        try {
            ...
            try {
                ...
                if (metadata.get() == null) {
                    return new DefaultResult(null, Result.Status.FAILURE, "There was an error processing metadata at OpenAirConnector at delete retrieving was successful but result is null");
                }
                return metadata;
            }
            catch (Exception e) {
